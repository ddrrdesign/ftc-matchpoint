...
function MobileEventCard({ r }: { r: EventBrowseListRow }) {
  return (
    <li className="px-4 py-4 sm:px-5">
      ...
      <p className="mt-2 text-base font-medium leading-snug text-white/92">
        {r.name}
      </p>
      {r.typeLine ? (
        <p className="mt-1 text-xs text-white/40">{r.typeLine}</p>
      ) : null}
      {r.divisionsNote ? (
        <p className="mt-1 text-[11px] text-amber-200/55">{r.divisionsNote}</p>
      ) : null}
      <p className="mt-2 text-sm text-white/65">{r.dates}</p>
      <p className="mt-1 text-sm leading-snug text-white/55">{r.location}</p>
      {r.venueExtra && r.venueExtra !== r.location ? (
        <p className="mt-0.5 text-xs text-white/38">{r.venueExtra}</p>
      ) : null}
      <p className="mt-2 text-xs text-white/45">
        Teams{" "}
        <span className="font-medium tabular-nums text-white/75">{r.teams}</span>
      </p>
...
export function EventBrowseList({ rows }: { rows: EventBrowseListRow[] }) {
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#06040f]/80 shadow-[0_0_0_1px_rgba(255,255,255,0.03)_inset] md:mt-10">
